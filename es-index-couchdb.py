"""
    Update ES index with values from CouchDB
...
def main():
    ...
    while True:
            ...
            if flag_start:
                flag_start = False
                last_seq_processed = last_seq_processed - 2*bulk_size #reindex a few already processed
                if last_seq_processed < 0:
                    last_seq_processed = 0
                print "Start from seq:",last_seq_processed
            ...
            #3 - update last sequence 
            computed_last_seq_processed = min(last_seq_processed + bulk_size, last_seq_reported)
            total_seq = db_couch.info()['update_seq']
            ...
            if last_seq_processed != computed_last_seq_processed:
                ...
                dbix[index_doc_seq] = indexdoc
                print "Index seq %d/%d" % (computed_last_seq_processed, total_seq),
                eps = 1e-6
                if time_delta < eps:
                    time_delta = eps
                rate = bulk_size / time_delta #values per seconds
                left_seconds = float(total_seq - computed_last_seq_processed) / rate
                hours = int(left_seconds / 60.0 / 60.0)
                minutes = int(left_seconds/60.0 - hours*60)
                seconds = int(left_seconds - minutes*60 - hours*60*60)
                print "- %.3f%% (%.2fs)  %.2f val/sec - ETA: %d:%d:%d" % (computed_last_seq_processed*100.0/total_seq, time_delta, rate, hours, minutes, seconds)

            if total_seq <= computed_last_seq_processed:
